          ...
          return (
            <div key={index} className={`flex items-center justify-between ${index > 0 ? 'border-t border-gray-200' : ''} pt-4`}>
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <p className="font-semibold">{departureTime} – {arrivalTime}</p>
                  <p className="text-sm text-gray-600">{leg.from} - {leg.to}</p>
                </div>
              </div>
              <div>
                <p className="font-semibold">{stopovers}</p>
                <p className="text-sm text-gray-600">{stopoverLocations}</p>
              </div>
